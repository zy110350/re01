...
const addons = require('./unimport.8553d39e.cjs');
...
function resolveGlobsExclude(glob2, cwd) {
  return `${glob2.startsWith("!") ? "!" : ""}${pathe.resolve(cwd, glob2.replace(/^!/, ""))}`;
}
function joinGlobFilePattern(glob2, filePattern) {
  return pathe.join(pathe.basename(glob2) === "*" ? pathe.dirname(glob2) : glob2, filePattern);
}
...
async function scanFilesFromDir(dir, options) {
  ...
  const indexOfDirs = (file) => dirGlobs.findIndex((glob2) => pm__default.isMatch(file, glob2));
